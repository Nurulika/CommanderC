...
#[derive(Debug)]
pub struct CommanderCLIProcessor {
    verbose: bool,
    processed_count: usize,
}

impl CommanderCLIProcessor {
...
pub fn run(verbose: bool, input: Option<String>, output: Option<String>) -> Result<()> {
    ...
    info!("Starting CommanderCLI processing");
    
    let mut processor = CommanderCLIProcessor::new(verbose);
...
mod tests {
    ...
    fn test_processor_creation() {
        let processor = CommanderCLIProcessor::new(true);
    ...
    fn test_data_processing() {
        let mut processor = CommanderCLIProcessor::new(false);
